...
pub fn part2(input: &str) -> i64 {
    ...
    let mut points: HashMap<Vec3<i64>, Point> = HashMap::with_capacity(num_points);
    ...
    while let Some(dist) = distances.pop() {
        ...
        match (c1_some, c2_some) {
            (false, false) => {
                let circ_idx = circuits.len();
                points.get_mut(&dist.v1).unwrap().circuit = Some(circ_idx);
                points.get_mut(&dist.v2).unwrap().circuit = Some(circ_idx);
                circuits.push(vec![dist.v1, dist.v2]);
                // impossible for these to be the final two junctions joined as they just created a
                // new circuit
            }
            (true, false) => {
                ...
                if circuits[circ].len() == num_points {
                    return points[&dist.v1].p.0 * points[&dist.v2].p.0;
                }
            }
            (false, true) => {
    ...
    unreachable!("Junctions did not connect into one big circuit!");
}
